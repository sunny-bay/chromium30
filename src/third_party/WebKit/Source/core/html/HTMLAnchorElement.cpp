...
bool HTMLAnchorElement::isVisible()
{
	for (Node* node = this; node; node = node->parentElement()) {
		if(node->computedStyle()&& node->computedStyle()->display() == NONE)
			return true;
	}
	return false;
}
void HTMLAnchorElement::defaultEventHandler(Event* event)
{
    if (isLink()) {
        if (focused() && isEnterKeyKeydownEvent(event) && treatLinkAsLiveForEventType(NonMouseEvent)) {
     		if(isVisible() ){ //for bug 2980,zhaoshuang,2016-11-17
				return;
			}
